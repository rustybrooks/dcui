...
class DockerTable(ScrollView, Generic[CellType], can_focus=True):
    ...
    def _update_dimensions(self, new_rows: Iterable[int]) -> None:
        ...
        for row_index in self.rows:
            for column_name, renderable in zip(self.display_columns, self._get_row_renderables(row_index)):
    ...
    def add_column(self, label: str, *, width: int | None = None) -> None:
        ...
        if width is None:
            ...
            column = Column(text_label, width, content_width=content_width, index=len(self.display_columns))
    ...
    def _get_row_renderables(self, row_index: int) -> list[RenderableType]:
        ...
        if data is None:
            ...
            return [default_cell_formatter(data[self.columns[d].index]) for d in self.display_columns]
            # return [
    ...
    def _render_line(self, y: int, x1: int, x2: int, base_style: Style) -> list[Segment]:
        ...
        fixed_width = sum(self.columns[column].render_width for column in self.display_columns[: self.fixed_columns])
    ...
    def render_line(self, y: int) -> list[Segment]:
        ...
        fixed_top_row_count = sum(self.get_row_height(row_index) for row_index in range(self.fixed_rows))
        if self.show_header:
    ...
    def _get_cell_border(self) -> Spacing:
        ...
        top += sum(self.rows[row_index].height for row_index in range(self.fixed_rows) if row_index in self.rows)
        left = sum(self.columns[column].render_width for column in self.display_columns[: self.fixed_columns])
...
class DockerComposeController(DockerTable):
    DEFAULT_CSS = """
